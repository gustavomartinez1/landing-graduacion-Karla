...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/graduates").then((r) => r.json()).then(setGraduates).catch(() => setError("Error al cargar datos")).finally(() => setLoading(false));
  }, [isLoggedIn]);
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      if (res.status === 401) {
        ...
        setError("Sesion expirada. Ingresa el password nuevamente."); return;
      }
    ...
    return (
      <div className="min-h-screen bg-[#0f172a] flex items-center justify-center px-4">
        <div className="bg-[#1e293b] p-8 rounded-2xl w-full max-w-md border border-gray-800">
          <h1 className="text-2xl font-serif text-[#d4af37] mb-6 text-center">Panel Admin</h1>
          <div className="space-y-4">
            <input type="password" placeholder="Password de administrador" value={password}
    ...
    <div className="min-h-screen bg-[#0f172a]">
      <div className="max-w-5xl mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-3xl font-serif text-[#d4af37]">Panel de Administracion</h1>
            <p className="text-gray-400 mt-1">{graduates.length} foto{graduates.length !== 1 ? "s" : ""} en la galeria</p>
          </div>
          <a href="/" className="text-[#d4af37] hover:text-[#f0d060] transition-colors underline underline-offset-4">Ver sitio publico →</a>
        </div>
        {error && <div className="bg-red-600/20 text-red-400 px-4 py-3 rounded-lg mb-6">{error}<button onClick={() => setError("")} className="ml-2 underline">Cerrar</button></div>}
