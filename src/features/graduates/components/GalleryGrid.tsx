...
interface GalleryGridProps { graduates: Graduate[]; }

export default function GalleryGrid({ graduates }: GalleryGridProps) {
      ...
      <div className="columns-1 sm:columns-2 lg:columns-3 gap-4 space-y-4">
        {graduates.map((g, i) => (
          <motion.div key={g.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            transition={{ delay: Math.min(i * 0.05, 1) }} className="break-inside-avoid">
