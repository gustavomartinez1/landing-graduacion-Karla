...
export default function AdminTable({ graduates, onDelete, deleting }: AdminTableProps) {
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-left">
        <thead>
          <tr className="border-b border-gray-700 text-gray-400 text-sm uppercase">
            <th className="pb-3 pr-4">Foto</th><th className="pb-3 pr-4">Nombre</th><th className="pb-3 pr-4">Fecha</th><th className="pb-3">Accion</th>
          </tr>
        </thead>
        <tbody>
          <AnimatePresence>
                  ...
                  {confirmId === g.id ? (
                    <div className="flex gap-2 items-center">
                      <button onClick={() => { onDelete(g.id); setConfirmId(null); }} disabled={deleting === g.id}
                        className="px-3 py-1.5 bg-red-600 text-white text-sm rounded-lg hover:bg-red-700 disabled:opacity-50 transition-colors">{deleting === g.id ? "..." : "Confirmar"}</button>
                      <button onClick={() => setConfirmId(null)} className="px-3 py-1.5 text-gray-400 text-sm hover:text-white transition-colors">Cancelar</button>
                    </div>
                  ) : (
                    <button onClick={() => setConfirmId(g.id)} className="px-3 py-1.5 bg-red-600/20 text-red-400 text-sm rounded-lg hover:bg-red-600/30 transition-colors">Eliminar</button>
                  )}
                </td>
              </motion.tr>
            ))}
      ...
      {graduates.length === 0 && <p className="text-center text-gray-500 py-8">No hay graduados registrados</p>}
    </div>
  );
}
