...
export default function UploadModal({ isOpen, onClose, onSuccess }: UploadModalProps) {
  ...
  const resetForm = () => { setFile(null); setPreview(null); setName(""); setErrors({}); reset(); };
  ...
  const validate = (): boolean => {
    ...
    if (!trimmed) errs.name = "Ingresa tu nombre completo";
    else if (trimmed.length < 2) errs.name = "El nombre debe tener al menos 2 caracteres";
      ...
      <div className="p-6">
        ...
        {state.step === "success" ? (
          ...
          <div className="space-y-4">
            ...
            <Input label="Nombre completo" placeholder="Ej: Maria Gonzalez Lopez" value={name}
              onChange={(e) => { setName(e.target.value); setErrors(p => ({ ...p, name: undefined })); }} error={errors.name} disabled={isProcessing} />
